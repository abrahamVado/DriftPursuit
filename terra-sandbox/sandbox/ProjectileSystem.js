...
export class ProjectileSystem {
  ...
  update(dt, { scenario = 'planet', originOffset = null, spaceBodies = [] } = {}){
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i -= 1){
      ...
      if (scenario === 'planet' && this.world){
        ...
        if (Number.isFinite(ground) && projectile.position.z <= ground + 2){
          ...
          impactPos.z = ground;
          this._spawnExplosion(impactPos, 1);
          const worldX = impactPos.x + origin.x;
          const worldY = impactPos.y + origin.y;
          this.world.addDynamicCrater?.({ worldX, worldY, radius: 320, depth: 220, rimHeight: 68 });
          this._disposeProjectileIndex(i);
          impacted = true;
        }
      }
      ...
      if (!impacted && scenario === 'space' && Array.isArray(spaceBodies)){
        for (let b = 0; b < spaceBodies.length; b += 1){
          ...
          if (distance <= radius){
            const impactPos = projectile.position.clone();
            this._spawnExplosion(impactPos, 0.6);
            this._disposeProjectileIndex(i);
            impacted = true;
            break;
          }
    ...
    for (let e = this.explosions.length - 1; e >= 0; e -= 1){
      ...
      const scale = THREE.MathUtils.lerp(1, explosion.scaleTarget, Math.pow(t, 0.6));
      explosion.mesh.scale.set(scale, scale, scale * 0.5);
  ...
  _spawnExplosion(position, size = 1){
    const mesh = new THREE.Mesh(this.shared.explosionGeometry.clone(), new THREE.MeshBasicMaterial({ color: 0xffc893, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending, depthWrite: false }));
